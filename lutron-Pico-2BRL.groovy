 ...
 *	v2.0.0.00	2018-04-22	wjh			Modified for PJ2-2BRL Picos
...
metadata {
	...
	tiles (scale: 2) {
        standardTile("mainfavorite", "favorite", width: 6, height: 1, canChangeIcon: true, canChangeBackground: true) {
			state "favorite", label: 'FAVE:[${currentValue}]', defaultState: true, nextState: "><", action: "pushFavorite", icon: "http://i66.tinypic.com/2uiydti.png"
			state "0", label: '', icon: "http://i66.tinypic.com/2uiydti.png"
		}
		...
		standardTile("btn3", "", width: 1, height: 2, decoration: "flat") {
			state "", label:'', backgroundColor: "#ffffff"
		}
        ...
        standardTile("picoIcon", "", width: 3, height: 3, decoration: "flat", canChangeIcon: true) {
			state "default", label:'', icon: "http://i66.tinypic.com/2uiydti.png", backgroundColor: "#ffffff"
		}
    ...
    preferences {
		...
		input name: "btn3Msg", type: "paragraph", title: "Button 3", element: "paragraph", description: "Button 3 mirrors Button 5"
...
def buttonEvent(button, action) {
    ...
    def btnList = [button]
	def mButton = btnA0PicoToBtnAMirror[button - 1]
    if (mButton)
		btnList << mButton
	def bEventList = []        
	btnList.each { b ->
		bEventList << createEvent(name: eAttrib, value: action, data: ((eAttrib == 'contact')?[contactNumber: b]:[buttonNumber: b]),
									descriptionText: "${device.label} $eAttrib $b was $action", isStateChange: true)
	}
    bEventList
}
...
def push(button) {
    ...
    log.info "Pico ${device.label} button $button event: action = $eValue"
   	if (sendThruLutron)
		...
		def mButton = btnA0PicoToBtnAMirror[button - 1]
	    if (mButton)
			sendEvent(name: eAttrib, value: eValue, data:  eData.collectEntries{ k, b -> [k, mButton] }, descriptionText: "${device.label} $eAttrib $mButton was $eValue", isStateChange: true)
	}
}
...
def reportPicoMode() {
    ...
    def timeoutMask = [	Pico2ButtonRaiseLower:		[true,true,false,false,false]
                      ]

    def holdTimeout = settings['holdTimeout']?:(metadata['preferences']['sections']['input'][0].find{p -> (p.name == 'holdTimeout')}?.defaultValue?:false)
	def toMask = timeoutMask[getDataValue('deviceType')] ?: [true,true,false,false,false];
...
// our app button (-1) to its mirror app button button code
def getBtnA0PicoToBtnAMirror() {
	[0,0,0,0,3]
}
...
def initialize() {
   if (state.label != device.label) {
	...
	def nButtons = device.currentValue("numberOfButtons")
    def faveB = (settings["faveButton"] && btnA0PicoToLPico[settings["faveButton"]-1]) ? settings["faveButton"] : 0
    sendEvent(name: "favorite", value: (faveB?:(metadata["preferences"]["sections"]["input"][0].find{p -> (p.name == "faveButton")}?.defaultValue?:0)))
